...
class BoardTest {
    ...
    void getSquareNumberAndName() {
        ...
        for (int i = 0; i < 40; ++i) {
            if (i == 0) {
                ...
                assertEquals("board.squares.Square " + i, board.getSquare(i).getName());
                assertEquals(i, board.getSquare(i).getNumber());
            }
        }
    }
    ...
    void getSquareNumberWithAdvancement() {
        ...
        Square old38 = new RegularSquare("board.squares.Square 38", 38);

        // normal case
        assertEquals(board.getSquare(old0, 10).getName(), "board.squares.Square 10");
        // (38 + 2) % 40 = 0 (Go)
        assertEquals(board.getSquare(old38, 2).getName(), "Go");
        // (38 + 3) % 40 = 1
        assertEquals(board.getSquare(old38, 3).getName(), "board.squares.Square 1");
    }
}